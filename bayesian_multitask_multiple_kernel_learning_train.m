function state = bayesian_multitask_multiple_kernel_learning_train(Km, y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        %%%% update upsilon
        for t = 1:T
            upsilon.beta(t) = 1 / (1 / parameters.beta_upsilon + 0.5 * (sum(diag(GtimesGT{t}.mu)) - 2 * sum(sum(reshape(a{t}.mu' * Km{t}, [N(t), P])' .* G{t}.mu)) + sum(diag(KmKm{t} * atimesaT{t}.mu))));
        end
        %%%% update a
        for t = 1:T
        ...
        %%%% update epsilon
        for t = 1:T
            epsilon.beta(t) = 1 / (1 / parameters.beta_epsilon + 0.5 * (y{t}' * y{t} - 2 * y{t}' * [ones(1, N(t)); G{t}.mu]' * be.mu([t, T + 1:T + P]) + N(t) * btimesbT.mu(t, t) + sum(diag(GtimesGT{t}.mu * etimeseT.mu)) + 2 * sum(G{t}.mu, 2)' * etimesb.mu(:, t)));
        end
        %%%% update b and e
        be.sigma = [diag(gamma.alpha .* gamma.beta) + diag(N .* epsilon.alpha .* epsilon.beta), zeros(T, P); zeros(P, T), diag(omega.alpha .* omega.beta)];
        for t = 1:T
        ...
        if parameters.progress == 1
            ...
            %%%% p(lambda)
            for t = 1:T
                lb = lb + sum((parameters.alpha_lambda - 1) * (psi(lambda{t}.alpha) + log(lambda{t}.beta)) - lambda{t}.alpha .* lambda{t}.beta / parameters.beta_lambda - gammaln(parameters.alpha_lambda) - parameters.alpha_lambda * log(parameters.beta_lambda));
            end
            %%%% p(upsilon)
            ...
            %%%% p(a | lambda)
            for t = 1:T
                lb = lb - 0.5 * sum(diag(diag(lambda{t}.alpha .* lambda{t}.beta) * atimesaT{t}.mu)) - 0.5 * (D(t) * log2pi - sum(psi(lambda{t}.alpha) + log(lambda{t}.beta)));
            end
            %%%% p(G | a, Km, upsilon)
            for t = 1:T
                lb = lb - 0.5 * sum(diag(GtimesGT{t}.mu)) * upsilon.alpha(t) * upsilon.beta(t) + (a{t}.mu' * KmtimesGT{t}.mu) * upsilon.alpha(t) * upsilon.beta(t) - 0.5 * sum(diag(KmKm{t} * atimesaT{t}.mu)) * upsilon.alpha(t) * upsilon.beta(t) - 0.5 * N(t) * P * (log2pi - (psi(upsilon.alpha(t)) + log(upsilon.beta(t))));
            end
            %%%% p(gamma)
            ...
            %%%% p(b | gamma)
            lb = lb - 0.5 * sum(diag(diag(gamma.alpha .* gamma.beta) * btimesbT.mu)) - 0.5 * (T * log2pi - sum(psi(gamma.alpha) + log(gamma.beta)));
            %%%% p(omega)
            ...
            %%%% p(e | omega)
            lb = lb - 0.5 * sum(diag(diag(omega.alpha .* omega.beta) * etimeseT.mu)) - 0.5 * (P * log2pi - sum(psi(omega.alpha) + log(omega.beta)));
            %%%% p(epsilon)
            ...
            %%%% p(y | b, e, G, epsilon)
            for t = 1:T
                lb = lb - 0.5 * (y{t}' * y{t}) * epsilon.alpha(t) * epsilon.beta(t) + (y{t}' * (G{t}.mu' * be.mu(T + 1:T + P))) * epsilon.alpha(t) * epsilon.beta(t) + sum(be.mu(t) * y{t}) * epsilon.alpha(t) * epsilon.beta(t) - 0.5 * sum(diag(etimeseT.mu * GtimesGT{t}.mu)) * epsilon.alpha(t) * epsilon.beta(t) - sum(G{t}.mu' * etimesb.mu(:, t)) * epsilon.alpha(t) * epsilon.beta(t) - 0.5 * N(t) * btimesbT.mu(t, t) * epsilon.alpha(t) * epsilon.beta(t) - 0.5 * N(t) * (log2pi - (psi(epsilon.alpha(t)) + log(epsilon.beta(t))));
            end
